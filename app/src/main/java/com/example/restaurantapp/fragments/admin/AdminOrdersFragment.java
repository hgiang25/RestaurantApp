...
public class AdminOrdersFragment extends Fragment implements StaffOrderAdapter.OnOrderActionListener {
    ...
    public void onPreparing(OrderModel order) {
        ...
        android.util.Log.d("AdminOrders", "=== onPreparing ===");
                ...
                missingRecipes -> {
                    ...
                    FirebaseService.getInstance().checkIngredientsAvailability(
                            ...
                            missingIngredients -> {
                                ...
                                deductIngredientsForOrder(order, success -> {
                                    if (!success) {
                                        Toast.makeText(
                                                getContext(),
                                                "❌ Trừ kho thất bại. Đơn chưa được chuyển trạng thái",
                                                Toast.LENGTH_LONG
                                        ).show();
                                        return;
                                    }
    ...
    public void onCancel(OrderModel order) {
        ...
        new AlertDialog.Builder(requireContext())
                ...
                .setPositiveButton("Hủy đơn", (dialog, which) -> {
                    ...
                    cancelOrder(order, reason); // ✅ truyền order
                })
    ...
    private void cancelOrder(OrderModel order, String reason) {
        ...
        FirebaseService.getInstance().cancelOrderWithRefund(
                order.getId(),
                order.getCustomerId(), // ✅ HOÀN ĐIỂM CHO KHÁCH
                reason,
                unused -> Toast.makeText(
                        getContext(),
                        "Đã hủy đơn & hoàn điểm cho khách",
                        Toast.LENGTH_SHORT
                ).show(),
                e -> Toast.makeText(
                        getContext(),
     ...
     */
    private void deductIngredientsForOrder(
            OrderModel order,
        ...
        for (Object itemObj : items) {
            ...
            if (menuItemId != null && !menuItemId.isEmpty() && quantity > 0) {
                pending[0]++;
            }
        }

        if (pending[0] == 0) {
            android.util.Log.d("AdminOrders", "Không có món hợp lệ để trừ kho");
            callback.accept(true);
            return;
        }

        // ✅ Runnable thay cho hàm checkFinish
        Runnable checkFinish = () -> {
            if (completed[0] >= pending[0] && !callbackCalled[0]) {
                callbackCalled[0] = true;
                callback.accept(!hasError[0]);
            }
        };
        ...
        for (Object itemObj : items) {
            ...
            android.util.Log.d(
                    "AdminOrders",
                    "Đang trừ kho: " + itemName + " x" + quantity
            );
            ...
            FirebaseService.getInstance().deductIngredientsFromRecipe(
                    ...
                    e -> {
                        ...
                        android.util.Log.e(
                                "AdminOrders",
                                "❌ Lỗi trừ kho (" + itemName + "): " + e.getMessage()
                        );
                        checkFinish.run();
                    }
            );
